...
# enc: utf-8
"""
Class for instantiating a hydrodynamics code with consistent settings
"""
...
class Hydro(object):
    ...
    def __init__(self, hydro_code, particles, stars):
        ...
        mass_of_cloud = particles.mass.sum()
        number_of_particles = len(particles)
        time_step = (
            0.004*numpy.pi * numpy.power(eps, 1.5)
            / numpy.sqrt(constants.G*mass_of_cloud/number_of_particles)
        )
        print("Hydro timesteps:", time_step, "N=", number_of_particles)
        ...
        self.star_attributes = [
            'name', 'birth_age', 'angular_momentum', 'mass',
        ...
        if hydro_code is Fi:
            ...
            self.code.parameters.timestep = 0.5*time_step
        ...
        if hydro_code is Gadget2:
            ...
            self.code.parameters.time_max = (
                time_step * 2**int(
                    numpy.log2(4 * (10 | units.Myr) / time_step)
                )
            )
            self.code.parameters.max_size_timestep = 0.5*time_step
    ...
    def write_set_to_file(self, index):
        ...
        write_set_to_file(self.gas_particles, filename,
                          "amuse", append_to_file=False)
        if not self.star_particles.is_empty():
            write_set_to_file(self.star_particles, filename, "amuse")
    ...
    def evolve_model(self, model_time):
        ...
        time_step = model_time - model_time_old
        print("Evolve Hydrodynamics:", time_step.in_(units.yr))

        if COOL:
            print("Cool gas for dt=", (time_step/2).in_(units.Myr))
            self.cooling.evolve_for(time_step/2)
            # print "...done."
        ...
        if COOL:
            print("Cool gas for another dt=", (time_step/2).in_(units.Myr))
            self.cooling.evolve_for(time_step/2)
            # print "...done."
        ...
        print(
            "final N=",
            len(self.star_particles), len(self.code.dm_particles))
        print(
            "final Ngas=",
            len(self.gas_particles), len(self.code.gas_particles))
    ...
    def resolve_sinks(self):
        ...
        if not candidate_stars.is_empty():
            ...
            print("pre N=", len(self.star_particles), len(
                newstars), len(self.code.dm_particles))
            # self.star_particles.add_sinks(newstars)
            self.star_particles.add_particles(newstars)
            print("post N=", len(self.star_particles), len(
                newstars), len(self.code.dm_particles))
        else:
    ...
    def merge_stars(self):
        ...
        for component in ccs:
            if len(component) > 1:
                nmerge += 1
            print("Merge stars: N= ", len(component))
            merge_two_stars(self.star_particles, component.copy())
