...
def make_ifft_real(nf, vi):
    if vi.ndim == 3:
        # body of cube
        vi[1:nf, 1:2*nf, 1:2*nf] = numpy.conj(
                vi[2*nf-1:nf:-1, 2*nf-1:0:-1, 2*nf-1:0:-1]
                )
...
def make_div_free(nf, vx, vy, vz):
    ...
    # inside the nyquist planes only v// f x f_mirror needs to be enforced
    # (methinks)
...
class molecular_cloud(object):
    ...
    def new_model(self):
        ...
        vx_field, vy_field, vz_field = make_div_free(
                self.nf, vx_field, vy_field, vz_field)

        base_sphere = uniform_unit_sphere(
                self.targetN, base_grid=self.base_grid)
...
class constant_density_div_free_power_law_v_ism_cube(object):
    def __init__(
            self, nf=32, power=-3., targetN=10000, eketh_ratio=1.,
    ...
    def result(self):
        ...
        if self.convert is not None:
            result = datamodel.ParticlesWithUnitsConverted(
                    result,
                    self.convert.as_converter_from_si_to_generic())
            result = result.copy()
...
def ism_cube(
    ...
    return constant_density_div_free_power_law_v_ism_cube(
            convert=convert, targetN=targetN, nf=nf, power=power, seed=seed,
            base_grid=base_grid, eketh_ratio=eketh_ratio)
...
if __name__ == "__main__":
    ...
    print(
            'dens:',
            (total_mass*3/4./3.1415/radius**3).in_(units.amu/units.cm**3)
            )
